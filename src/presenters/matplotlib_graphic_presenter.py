...
class MatplotlibGraphicsPresenter(MultiplyDataPresenter):
    ...
    def _get_shots_and_points_bars(self, ax: Axes, data: DataFrame) -> None:
        groups = [
            (data["shots_1"], SHOTS_1_NAME, SHOTS_1_COLOR),
            (data["points_1"], POINTS_1_NAME, POINTS_1_COLOR),
            (data["shots_2"], SHOTS_2_NAME, SHOTS_2_COLOR),
            (data["points_2"], POINTS_2_NAME, POINTS_2_COLOR),
            (data["shots_3"], SHOTS_3_NAME, SHOTS_3_COLOR),
            (data["points_3"], POINTS_3_NAME, POINTS_3_COLOR),
        ]
        self._plot_grouped_bars(ax, data, groups)
        ax.set_xticks(np.arange(len(data)))
        ax.set_xticklabels(data["game_info"], rotation=45, ha="right", fontsize=10)
        ax.legend()
